...
class GroqService {
  ...
  static String get _defaultApiKey =>
      dotenv.env['GROQ_API_KEY'] ?? ''; //api key
  ...
  static Future<Map<String, dynamic>> checkGrammar(String text) async {
    try {
      ...
      if (_responseCache.containsKey(cacheKey) &&
          !_responseCache[cacheKey]!.isExpired) {
      ...
      if (errorMessage.contains('SocketException') ||
        ...
        errorMessage =
            'API key error. Please check your Groq API key in settings.';
      }
  ...
  static Future<Map<String, dynamic>> translateText(
    ...
    try {
      ...
      print(
          'Sending translation request to Groq for language: $targetLanguage');
      final response = await http.post(
        Uri.parse('$_baseUrl/chat/completions'),
        headers: {
          'Content-Type': 'application/json',
