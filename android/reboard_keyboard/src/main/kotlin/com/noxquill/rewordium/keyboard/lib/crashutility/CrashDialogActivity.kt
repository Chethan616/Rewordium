...
package com.noxquill.rewordium.keyboard.lib.crashutility
...
import com.noxquill.rewordium.keyboard.lib.devtools.Devtools
...
class CrashDialogActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val versionName = buildString {
            append("[")
            append(ReboardInfo.VERSION_NAME)
            append("](")
            if (BuildConfig.DEBUG) {
                append(stringRes(R.string.florisboard__commit_by_hash_url, "hash" to BuildConfig.BUILD_COMMIT_HASH))
            } else {
                append(stringRes(R.string.florisboard__changelog_url, "version" to ReboardInfo.VERSION_NAME))
            }
            append(")")
        }
        errorReport.apply {
            appendLine("#### Environment information")
            appendLine("- FlorisBoard $versionName (${ReboardInfo.VERSION_CODE})")
