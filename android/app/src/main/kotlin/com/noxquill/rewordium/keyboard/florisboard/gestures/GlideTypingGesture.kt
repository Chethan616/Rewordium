...
class GlideTypingGesture {
    ...
    class Detector(context: Context) {
        ...
        private val keySize = dpToPx(context, 48f) // Approximate key size in dp
        private val listeners: ArrayList<Listener> = arrayListOf()
        private var pointerId: Int = -1

        companion object {
            private const val TAG = "GlideTypingGesture"
            private const val MAX_DETECT_TIME = 500
            private const val VELOCITY_THRESHOLD = 0.10 // dp per ms
            
            // Key codes that should not trigger glide typing
            private val SWIPE_GESTURE_KEYS = setOf(
                -7,  // DELETE
                -11, // SHIFT
                32   // SPACE
            )
         ...
         */
        fun onTouchEvent(event: MotionEvent, initialKeyCode: Int? = null): Boolean {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    for (i in 0..event.historySize) {
                        ...
                        if (pointerData.isActuallyGesture == null) {
                            ...
                            val context = android.os.Looper.getMainLooper().thread.contextClassLoader?.loadClass("android.content.Context")
                            val distPx = pointerData.positions[0].dist(pos)
                            val dist = distPx / android.content.res.Resources.getSystem().displayMetrics.density
                            val time = (System.currentTimeMillis() - pointerData.startTime) + 1
                            Log.d(TAG, "Distance glided: $dist dp with velocity: ${dist / time} dp/ms")
                            if (dist > keySize && (dist / time) > VELOCITY_THRESHOLD && (initialKeyCode !in SWIPE_GESTURE_KEYS)) {
