// ReboardStyle - Shared utilities for build scripts
...
const unzipFlex = async (flexFile, outDir) => {
	...
	logInfo(`Decompressed ${flexFile} -> ${outDir}`)
}
...
const updateVersion = async (dir, version) => {
	...
	if (!ext.meta) throw new Error('"meta" not found in extension.json')
...
const zipFolder = async (folder, outFile) => {
	...
	logInfo(`Packaged ${folder} -> ${outFile}`)
}

module.exports = { logInfo, logWarn, logError, logOk, updateVersion, zipFolder, unzipFlex }
