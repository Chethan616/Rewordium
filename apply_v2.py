...
replacement_buttons = '''ToolButton(
                        ...
                        ToolButton(
                            ...
                            onClick = {
                                ...
                                scope.launch {
                                    val ok = addStickerOutline(editor)
...
replacement_text_dialog = '''if (showTextDialog) {
        ...
        val aiManager = remember { com.noxquill.rewordium.keyboard.ime.media.sticker.AiManager.get(context) }
                    ...
                    Row(
                        ...
                        TextButton(
                            onClick = {
                                aiLoading = true
                                scope.launch {
                                    val res = aiManager.rewriteTextWithPrompt("Generate a short, funny meme caption suitable for a sticker (max 4 words). Output just the caption without quotes.")
                                    aiLoading = false
...
private suspend fun addStickerOutline(editor: ja.burhanrashid52.photoeditor.PhotoEditor): Boolean {
    return withContext(Dispatchers.Default) {
        try {
            val view = editor.photoEditorView ?: return@withContext false
            ...
            withContext(Dispatchers.Main) {
                ...
                editor.photoEditorView?.source?.setImageBitmap(out)
            }
            true
        } catch (e: Exception) {
            e.printStackTrace()
